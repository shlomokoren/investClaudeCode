...
- **`templates/base.html`** — shared layout (tab nav + `<head>`/Chart.js include). The nav renders tabs, the signed-in email, and a POST sign-out form only when `current_user` is set, so `login.html` can extend the same base without them. `price.html`, `financials.html`, and `login.html` extend it and fill the `content`/`scripts` blocks. All styling lives in the single `public/css/style.css` (no per-page stylesheets).
...
- `render.yaml` (Render, `gunicorn app:app`, health check on `/healthz`; `SECRET_KEY` uses `generateValue: true`, the rest are `sync: false` so values are set in the dashboard). `startCommand` runs plain sync workers (`--workers 2`) — fine for Render's own low concurrency, but undersized if traffic ever needs many requests in flight at once (each ticker fetch is a blocking network call); bump `--workers`/add `--threads` before relying on this under real concurrent load.
...
- **Vercel** (`vercel.json`) — Python runtime, zero-config: Vercel detects the top-level `app` Flask instance in `app.py` and runs the whole app as one serverless function (Fluid Compute), so no route-by-route restructuring is needed. `vercel.json` sets `maxDuration: 60` for `app.py` since sequential per-ticker yfinance calls can take a few seconds. Because instance count can scale far more elastically than a fixed number of gunicorn workers, `DATABASE_URL` should point at Neon's **pooled** connection string (`-pooler` host) when deployed here, to avoid exhausting Postgres connections under concurrent invocations — the direct string works locally and on Render precisely because those have few, fixed workers. In-memory caches (`symbols.py`'s 60s global-config cache, `blueprints/financials.py`'s 24h cache) still work but are only as durable as the underlying function instance — expect more cache misses than on Render's single long-lived process. `DEBUG` must stay unset/false, same as any other deployment.

Static assets live in `public/`, not `static/` — `app.py` sets `static_folder="public", static_url_path=""` so Flask serves them at the site root (`/css/style.css`, not `/static/css/style.css`) both locally and on Render. On Vercel, files under `public/**` are instead served directly by its CDN rather than passing through the function, which is why the folder has to be named exactly `public` there; the matching Flask config keeps local/Render behavior identical without any template changes (`url_for('static', ...)` still resolves correctly).
